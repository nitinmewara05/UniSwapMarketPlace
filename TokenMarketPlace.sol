...
contract customDex{
    string[] public tokens = ["BNB", "Tether USD", "USD Coin", "stETH", "TRON", "MATIC", "SHIBA", "Uniswap"];
...
function swarEthToToken (string memory tokenName) public payable returns (uint256) {
...
function swapTokenToToken(string memory srcTokenName, string memory exactTokenName, string memory desTokenName, uint256 _amount) public {
    require(tokenInstanceMap[srcTokenName].transferFrom(msg.sender, address(this), _amount));
    require(tokenInstanceMap[desTokenName].transfer(msg.sender, _amount));
}
